...
def IKinBodyIterates(Blist, M, T, thetalist0, eomg, ev,
    ...
    with output info at each iteration
    ...
    :param filename:output joint value at each iteration
    ...
    In short the new IKinBody function is based on  origin IKinBody from MR library
    """

    '''
    print iteration info to console
    '''
    def PrintIteration(i, thetalist, Tb, Vb, eAng, eLin):
        print('Iteration %d' %i)
        print('joint vector: %s' %thetalist)
        print('end effector configuration:')
        print('%s' %Tb)
        print('twist error: %s' %Vb)
        print('angular error magnitude: %s' %eAng)
        print('linear error magnitude: %s' %eLin)
        print('')

    '''
    save iteration joint value to csv
    '''
    def SaveJointValue(f, thetalist):
    ...
    while err and i < maxiterations:
        ...
        eAng = np.linalg.norm([Vb[0], Vb[1], Vb[2]]) # twist angular error
        eLin = np.linalg.norm([Vb[3], Vb[4], Vb[5]]) # twist linear error
        err = eAng > eomg or eLin > ev
        PrintIteration(i, thetalist, Tb, Vb, eAng, eLin)
        SaveJointValue(f, thetalist)
        i += 1
    f.close()
